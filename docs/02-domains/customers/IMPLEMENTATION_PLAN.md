...
| --- | --- | --- | --- |
| Customer Relationship authority | dedicated tenant-scoped relationship identity, lifecycle, grant, reconciliation, audit and conservative legacy behavior | PLANNED | `CUSTOMER_RELATIONSHIP_DOMAIN_CHANGE_GATE.md`: tenant uniqueness, concurrent reconciliation, opaque denial, migration inventory/cutover, retention and rollback evidence; **out:** Directory, Overview, summaries, query and frontend |
| Customers Directory and Overview | tenant relationship directory, summaries, search, filters, selection, Profile/address read view | PLANNED | Relationship-authority unit plus **product semantics closed:** Profile, lifecycle/access, read authorization/freshness, summaries and directory query/cursor/selection. Still verify projection/cursor/search/filter enforcement, calculation/transport, Profile source/freshness, opaque denial and integration evidence |
